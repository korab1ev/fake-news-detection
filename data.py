...
def read_data(cfg: dict) -> Tuple[dict, dict, dict]:
    ...
    labeled_df = pd.read_csv(cfg['data']['labeled_df_filename'], sep="\t")
    
    # Perform a 0.9 / 0.1 split
    train_size = int(0.9 * len(labeled_df))
    val_size = len(labeled_df) - train_size

    # First to split df to train / test and then split train to validation / train.
    
    train_df, val_df = train_test_split(labeled_df, train_size=train_size, random_state=42)
    
    # Creating PyTorch Datasets
    train_dataset = TextClassificationDataset(
        texts=train_df['title'].values.tolist(),
        labels=train_df['is_fake'].values,
        max_seq_length=cfg['model']['max_seq_length'], 
        model_name=cfg['model']['model_name'],
    )

    val_dataset = TextClassificationDataset(
        texts=val_df['title'].values.tolist(),
        labels=val_df['is_fake'].values,
        max_seq_length=cfg['model']['max_seq_length'], 
        model_name=cfg['model']['model_name'],
    )
    ...
    # Now moving on to unlabeled set
    unlabeled_df = pd.read_csv(cfg['data']['unlabeled_df_filename'], sep="\t")
